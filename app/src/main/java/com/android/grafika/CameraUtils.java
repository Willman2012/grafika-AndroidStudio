...
public class CameraUtils {
    ...
    public static void choosePreviewSize(Camera.Parameters parms, int width, int height) {
        ...
        for (Camera.Size size : parms.getSupportedPreviewSizes()) {
            Log.d(TAG, "supported PreviewSizes: " + size.width + "x" + size.height);
        }
    ...
    /**
     * Attempts to find a picture size that matches the provided width and height (which
     * specify the dimensions of the encoded video).  If it fails to find a match it just
     * uses the default picture size for video.
     * <p>
     * TODO: should do a best-fit match, e.g.
     * https://github.com/commonsguy/cwac-camera/blob/master/camera/src/com/commonsware/cwac/camera/CameraUtils.java
     */
    public static void choosePictureSize(Camera.Parameters parms, int width, int height) {
        ...
        Camera.Size ppsfv = parms.getPreferredPreviewSizeForVideo();
        if (ppsfv != null) {
            Log.d(TAG, "Camera preferred picture size for video is " +
                    ppsfv.width + "x" + ppsfv.height);
        }

        for (Camera.Size size : parms.getSupportedPictureSizes()) {
            Log.d(TAG, "supported PictureSizes: " + size.width + "x" + size.height);
        }
        ...
        Log.w(TAG, "Unable to set picture size to " + width + "x" + height);
        if (ppsfv != null) {
